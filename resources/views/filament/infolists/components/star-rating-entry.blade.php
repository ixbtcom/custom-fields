...
<x-dynamic-component :component="$getEntryWrapperView()" :entry="$entry">
    <div class="cf-star-rating cf-star-rating--readonly">
        @if ($state === null)
            <div class="cf-star-rating__stars" aria-hidden="true">
                @for ($i = 1; $i <= $stars; $i++)
                    <span class="cf-star-rating__star">
        ...
        @else
            <div class="cf-star-rating__stars" role="img" aria-label="{{ number_format($state, 1) }} / {{ number_format($max, 0) }}">
                @for ($i = 1; $i <= $stars; $i++)
                    <span class="cf-star-rating__star">
                        <svg class="cf-star-rating__star-bg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                            <path d="M12 2.5l2.95 6.6 7.05.8-5.25 4.95 1.55 7.15-6.3-3.6-6.3 3.6 1.55-7.15L2 9.9l7.05-.8L12 2.5z" />
            ...
            <span class="cf-star-rating__value">{{ rtrim(rtrim(number_format($state, 1, '.', ''), '0'), '.') }} / {{ rtrim(rtrim(number_format($max, 1, '.', ''), '0'), '.') }}</span>
        @endif
    </div>
</x-dynamic-component>
