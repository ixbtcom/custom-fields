...
return [
        ...
        'sections' => [
            ...
            'settings' => [
                ...
                'fields' => [
                    ...
                    'storage-helper' => 'Режим Schema добавляет колонку в таблицу. Режим JSON сохраняет значение в существующую JSON-колонку.',

                    'storage-options' => [
                        'schema' => 'Schema (отдельная колонка)',
                        'json'   => 'JSON (ключ в существующей колонке)',
                    ],

                    'storage-column'         => 'JSON-колонка',
                    'storage-column-helper'  => 'Имя существующей JSON-колонки на целевой модели (например «extra»). Должна быть в $fillable и кастоваться как array/json.',
                    'storage-column-warning' => 'Предупреждение: указанная колонка может отсутствовать на модели или не кастоваться как JSON. Значения могут не сохраняться корректно.',
                    'collision-warning'      => 'Предупреждение: ключ «:code» уже существует в колонке «:column» в некоторых записях. Создание этого поля может конфликтовать с существующими данными.',
    ...
    'table' => [
        ...
        'actions' => [
            'purge' => [
                'label' => 'Очистить значения',

                'modal' => [
                    'heading'     => 'Очистить значения поля',
                    'description' => 'Будут удалены значения поля «:code» из :count записи(-ей). Введите «:code» для подтверждения.',
                ],

                'confirmation' => [
                    'input-label' => 'Введите код поля для подтверждения',
                ],

                'notification' => [
                    'title' => 'Значения очищены',
                    'body'  => ':count запись(-ей) обновлено.',
                    'error' => 'Не удалось очистить значения.',
                ],

                'unsupported-driver' => 'Очистка JSON поддерживается только на MySQL. Операция отменена.',
            ],
        ...
        'bulk-actions' => [
            'purge' => [
                ...
                'modal' => [
                    'heading' => 'Очистить значения полей (массово)',
                ],

                'notification' => [
                    'title' => 'Массовая очистка завершена',
                    'body'  => 'Обновлено :updated записи(-ей), ошибок :failed, пропущено :skipped.',
                ],
            ],
